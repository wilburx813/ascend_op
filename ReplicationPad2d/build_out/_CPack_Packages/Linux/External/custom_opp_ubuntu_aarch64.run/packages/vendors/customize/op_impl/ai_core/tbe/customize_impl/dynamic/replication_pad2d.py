...
def _build_args(x_in__, paddings_in__, y_out_):
    __inputs__ = []
    for arg in [x_in__, paddings_in__]:
        if arg != None:
...
@tbe_register.register_operator("ReplicationPad2d", trans_bool_to_s8=False)
...
def replication_pad2d(x_in__, paddings_in__, y_out_, kernel_name="replication_pad2d", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    ...
    origin_func_name = "replication_pad2d"
    ascendc_src_dir_ex = "replication_pad2d"
    ascendc_src_dir = "replication_pad2d"
    ascendc_src_file = "replication_pad2d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ReplicationPad2d. kernel name is " + kernel_name)
    op_type = "ReplicationPad2d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, paddings_in__], origin_outputs = [y_out_],\
...
def op_select_format(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("op_select_format", "ReplicationPad2d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, paddings_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, paddings_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "ReplicationPad2d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
