...
def _build_args(x_in__, y_out_, dim):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
...
@tbe_register.register_operator("Softmax", trans_bool_to_s8=False)
...
def softmax(x_in__, y_out_, dim=-1, kernel_name="softmax", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim)
    ...
    origin_func_name = "softmax"
    ascendc_src_dir_ex = "softmax"
    ascendc_src_dir = "softmax"
    ascendc_src_file = "softmax.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Softmax. kernel name is " + kernel_name)
    op_type = "Softmax"
...
def op_select_format(x_in__, y_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim)
    result = check_op_cap("op_select_format", "Softmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_out_, dim=-1, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_out_, dim)
    result = check_op_cap("get_op_specific_info", "Softmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
