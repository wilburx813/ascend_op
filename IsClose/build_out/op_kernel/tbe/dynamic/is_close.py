...
def _build_args(x1_in__, x2_in__, y_out_, rtol, atol, equal_nan):
    __inputs__ = []
    ...
    if equal_nan != None:
        ...
        attr["name"] = "equal_nan"
        attr["dtype"] = "bool"
        attr["value"] = equal_nan
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("IsClose", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_FLOAT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def is_close(x1_in__, x2_in__, y_out_, rtol=1e-05, atol=1e-08, equal_nan=False, kernel_name="is_close", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, rtol, atol, equal_nan)
    ...
    origin_func_name = "is_close"
    ascendc_src_dir_ex = "is_close"
    ascendc_src_dir = "is_close"
    ascendc_src_file = "is_close.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator IsClose. kernel name is " + kernel_name)
    op_type = "IsClose"
...
def op_select_format(x1_in__, x2_in__, y_out_, rtol=1e-05, atol=1e-08, equal_nan=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, rtol, atol, equal_nan)
    result = check_op_cap("op_select_format", "IsClose", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, rtol=1e-05, atol=1e-08, equal_nan=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_, rtol, atol, equal_nan)
    result = check_op_cap("get_op_specific_info", "IsClose", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
