...
def _build_args(y_in__, dy_in__, z_out_):
    __inputs__ = []
    for arg in [y_in__, dy_in__]:
        if arg != None:
...
@tbe_register.register_operator("AsinhGrad", trans_bool_to_s8=False)
...
def asinh_grad(y_in__, dy_in__, z_out_, kernel_name="asinh_grad", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, dy_in__, z_out_)
    ...
    origin_func_name = "asinh_grad"
    ascendc_src_dir_ex = "asinh_grad"
    ascendc_src_dir = "asinh_grad"
    ascendc_src_file = "asinh_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator AsinhGrad. kernel name is " + kernel_name)
    op_type = "AsinhGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[y_in__, dy_in__], origin_outputs = [z_out_],\
...
def op_select_format(y_in__, dy_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, dy_in__, z_out_)
    result = check_op_cap("op_select_format", "AsinhGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(y_in__, dy_in__, z_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(y_in__, dy_in__, z_out_)
    result = check_op_cap("get_op_specific_info", "AsinhGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
