...
@tbe_register.register_operator("NotEqual", trans_bool_to_s8=False)
...
def not_equal(x1_in__, x2_in__, y_out_, kernel_name="not_equal", impl_mode=""):
    ...
    origin_func_name = "not_equal"
    ascendc_src_dir_ex = "not_equal"
    ascendc_src_dir = "not_equal"
    ascendc_src_file = "not_equal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator NotEqual. kernel name is " + kernel_name)
    op_type = "NotEqual"
...
def op_select_format(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("op_select_format", "NotEqual", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1_in__, x2_in__, y_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1_in__, x2_in__, y_out_)
    result = check_op_cap("get_op_specific_info", "NotEqual", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
